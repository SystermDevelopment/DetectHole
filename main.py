...
def capture_from_cam(video_path, start_frame=0):
    ...
    with open(get_log_path(), 'a') as log:
        while True:
            ...
            # filename = os.path.join('img_detect', f"org_{timestamp}.jpg")
            ...
            if INFERENCE:
              ...
              circles = cv.HoughCircles(img_canny, cv.HOUGH_GRADIENT,
                ...
                circles, img_canny, edge_points = detect_holes.circle_ransac(imgHD)
            ...
            if circles is not None:
                ...
                for cx, cy, r in circles.squeeze(axis=0).astype(int):
                    ...
                    if cx < CX_MIN_LIMIT or cx > CX_MAX_LIMIT:
                        ...
                        if check_count > 0:
                          ...
                          if INFERENCE:
                            ...
                            holes_exist = False
                            if (holes_ng_count / check_count) < HOLES_EXIST_OK_RATIO:
                                holes_exist = True
                            ...
                            if not (holes_exist):
                                ...
                                write_log("[detect] ------ detNG ----------", log)
                                write_log("[detect] check_count:%d" % (check_count), log)
                                write_log("[detect] holes:%s" % (hole1_exist), log)

                                print("detNG")
                                print("[detect] holes:%s" % (holes_exist))

                            print("holes:%d/%d => %.3f" % (holes_ng_count, check_count, holes_ng_count / check_count))
                            print("--------------------------")
                            write_log("holess:%d/%d => %.3f" % (holes_ng_count, check_count, holes_ng_count / check_count), log)
                            write_log("--------------------------", log)
                            log.flush()
                    ...
                    if INFERENCE:
                      ...
                      # 外線
                      cv.circle(imgHD, (int(cx), int(cy)), int(r) + detect_holes.OUTER_RINGSIZE, (0, 255, 0), 2)
                      # 内線
                      cv.circle(imgHD, (int(cx), int(cy)), int(r) + detect_holes.INNER_RINGSIZE, (0, 255, 0), 2)
                    ...
                    if INFERENCE:
                      try:
                        ...
                        cv.imwrite(filename, imgHD)
                        s = time.time()
                        hole_str = detect_holes.detect_holes(imgHD, edge_points, cx, cy, r,img_canny, filename) # 二値画像結果画像出力
                        elapse = time.time() - s

                        output = "%08d %s, %d, %dms" % (frame, hole_str, cx, elapse*1000)
